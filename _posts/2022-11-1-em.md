...
date: 2022-11-02
description: By Teng Gao, Nov. 2nd 2022
...
Expectation-maximization (EM) is a powerful class of statistical algorithms for performing inference in the presence of latent (unobserved) variables. There are many variations of EM being applied to solve different problems (e.g. K-means, gaussian mixture, Baum-Welch, LDA, you name it). However, it is often unclear how to derive an EM algorithm, from scrtach, for a new problem. In this tutorial, we will first derive and prove the *general* EM framework, which reveals the fundamental principles that all EM algorithms share. Then, we will use an example (mixture of exponentials) to illustatrate how to derive a *specific* EM algorithm using this general framework for any problem.
...
The E-step is in some way also a maximization step, because choosing $q(z) = p(z \| x,\theta^{(t)})$ maximizes the ELBO at $\theta = \theta^{(t)}$ (in fact, it is exatly equal to $l(\theta^{(t)})$, the log marginal likelihood at $\theta = \theta^{(t)}$). In this way, EM can be viewed as a special case of  [coordinate ascent](https://en.wikipedia.org/wiki/Coordinate_descent).
...
Where the first inequality is because ELBO is always a lower bound of $l$, the second is because we choose the new $\theta$ to be the maximizer of the current ELBO, and the final inequality is that $q_t$ was chosen so that the current ELBO is exactly equal to $l$ at the current $\theta$. Therefore, each time we update $\theta$ (M-step) we attain a higher marginal likelhood.
...
### Example derivation
Let's go over a concrete example to see how EM derivation works in action. Let's say we are trying to infer the failure rates of light bulbs coming from different manufacturing batches. Let $x_i$ be the observed failture time of lightbulb $i \in \{1\ldots n\}$, and $z_i \in \{1, \ldots, K\}$ be the batch that lightbulb $i$ came from (which we do not observe). Let's model the failture time using an exponential distribution, where the failture rate is different for each batch $k$:
...
To make the problem more challending, let's say both $\pmb{\lambda} = (\lambda_1, \lambda_2, \ldots, \lambda_K)$ and $\pmb{\pi} = (\pi_1, \pi_2, \ldots, \pi_K)$ are unknown. Let's denote the collection of these model parameters as $\Theta = (\pmb{\lambda}, \pmb{\pi})$. Well, how do we figure out the failture rates of different light bulb batches, if the batch labels themselves are unknown? Let's see how we can derive an EM algorithms that can achieve this. 
...
Where we have defined $\phi^t_{z_i}(k) = p(z_i=k\|x_i,\Theta^{(t)})$, which is intuitively the membership probability that lightbulb $x_i$ belongs to batch $k$. Note that this quantiy does not dependent on $\Theta$ (it only depends on $\Theta^{(t)}$, which is treated as a known constant). Also, $\sum_k \phi^t_{z_i}(k) = 1$ since it is a conditional probability. We can compute it using Bayes' Rule:
